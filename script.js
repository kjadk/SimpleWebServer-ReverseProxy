...
  // --- START MODIFIED LOG LINE for Rate Limiting ---
  // Changed '2-digit' to 'numeric' for day and month to remove leading zeros
  const formattedDate = new Date(currentTime).toLocaleDateString('no-NO', { day: 'numeric', month: 'numeric', year: 'numeric' });
  const formattedTime = new Date(currentTime).toLocaleTimeString('no-NO', { hour: '2-digit', minute: '2-digit', second: '2-digit' });
  // The original request URL is used here, as per your desired format
  console.log(`[${formattedDate}, ${formattedTime}] ${clientIp}: Request ${req.method} ${req.url}`); 
...
  // --- START MODIFIED LOG LINE for Proxying ---
  // The target URL is constructed using matchedRoute.target and proxyPath
  console.log(`[Proxying] -> ${matchedRoute.target}${proxyPath}`);
  // --- END MODIFIED LOG LINE ---
